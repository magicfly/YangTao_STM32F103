...
void set_led2(FunctionalState state){
	//@brief turn on or off led1
	//@param state on or off to be set to led2
	if(state == ENABLE){
...
void breathing_led(uint32_t count){
	//@brief simulate breathing via led1 and led2
	//@param count breathing times
	uint8_t bool_flag = 1;
	uint8_t i;
	uint16_t t_counter = 1;
	while(count--){
		if (bool_flag == 1){
			//led1 more bright, led2 less bright
			for(i = 0; i < 10; i++){
				set_led1(ENABLE);
				set_led2(DISABLE);
				delay_us(t_counter);
		...
		if (bool_flag == 0){
			////led1 less bright, led2 more bright
			for(i = 0; i < 10; i++){
				set_led1(ENABLE);
				set_led2(DISABLE);
				delay_us(t_counter);

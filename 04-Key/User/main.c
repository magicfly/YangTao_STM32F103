...
int main(void){
	...
	//uint8_t MENU = 6;
	uint8_t counter = 0;
	uint8_t debounce_counter = 10;
	uint8_t current_state =  GPIO_ReadInputDataBit(KEY_PORT, KEY1_Pin);
	set_led1((FunctionalState) current_state);
	while(1){
//		switch (MENU){
...
//		}
		delay_ms(1);
		uint8_t reading = GPIO_ReadInputDataBit(KEY_PORT, KEY1_Pin);
		...
		if(counter > debounce_counter){
			counter = 0;
			current_state = reading;
			set_led1((FunctionalState) reading);
		}
	}
}
